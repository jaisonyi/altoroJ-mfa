...
**
	...
	private static Connection getConnection() throws SQLException{
		...
		instance.ensureMfaSchema();

		return instance.connection;
	}

	/*
	 * Ensure the MFA columns exist on the PEOPLE table. This upgrades databases
	 * that were created before TOTP/MFA support was added without requiring a
	 * full re-initialization. Runs once per JVM and is safe to call repeatedly.
	 */
	private void ensureMfaSchema(){
		if (schemaChecked)
			return;
		...
		try {
			...
			try {
				statement.execute("ALTER TABLE PEOPLE ADD COLUMN TOTP_SECRET VARCHAR(64)");
			} catch (SQLException e) {
				// column already exists - not a problem
			}
			try {
				statement.execute("ALTER TABLE PEOPLE ADD COLUMN MFA_ENABLED SMALLINT NOT NULL DEFAULT 0");
			} catch (SQLException e) {
				// column already exists - not a problem
			}
		} catch (SQLException e) {
			Log4AltoroJ.getInstance().logError("Error ensuring MFA schema: " + e.getMessage());
		}
	...
	private void initDB() throws SQLException{
		...
		statement.execute("CREATE TABLE PEOPLE (USER_ID VARCHAR(50) NOT NULL, PASSWORD VARCHAR(20) NOT NULL, FIRST_NAME VARCHAR(100) NOT NULL, LAST_NAME VARCHAR(100) NOT NULL, ROLE VARCHAR(50) NOT NULL, TOTP_SECRET VARCHAR(64), MFA_ENABLED SMALLINT NOT NULL DEFAULT 0, PRIMARY KEY (USER_ID))");
	...
	/**
	 * Retrieve the stored TOTP shared secret for a user.
	 * @param username user name
	 * @return the Base32-encoded secret, or null if none is stored
	 * @throws SQLException
	 */
	public static String getTotpSecret(String username) throws SQLException{
		if (username == null || username.trim().length() == 0)
			return null;

		Connection connection = getConnection();
		Statement statement = connection.createStatement();
		ResultSet resultSet = statement.executeQuery("SELECT TOTP_SECRET FROM PEOPLE WHERE USER_ID = '"+ username +"' "); /* BAD - user input should always be sanitized */

		if (resultSet.next())
			return resultSet.getString("TOTP_SECRET");
	 ...
	 * @param username user name
	 * @param secret Base32-encoded secret, or null to clear
	 * @throws SQLException
	 */
	public static void setTotpSecret(String username, String secret) throws SQLException{
		Connection connection = getConnection();
		Statement statement = connection.createStatement();
		String value = (secret == null) ? "NULL" : "'" + secret + "'";
		statement.execute("UPDATE PEOPLE SET TOTP_SECRET = "+ value +" WHERE USER_ID = '"+ username +"'"); /* BAD - user input should always be sanitized */
	}

	/**
	 * Determine whether MFA is enabled (enrollment completed) for a user.
	 * @param username user name
	 * @return true if MFA is enabled, false otherwise
	 * @throws SQLException
	 */
	public static boolean isMfaEnabled(String username) throws SQLException{
		if (username == null || username.trim().length() == 0)
			return false;

		Connection connection = getConnection();
		Statement statement = connection.createStatement();
		ResultSet resultSet = statement.executeQuery("SELECT MFA_ENABLED FROM PEOPLE WHERE USER_ID = '"+ username +"' "); /* BAD - user input should always be sanitized */

		if (resultSet.next())
			return resultSet.getInt("MFA_ENABLED") != 0;
	 ...
	 * @throws SQLException
	 */
	public static void setMfaEnabled(String username, boolean enabled) throws SQLException{
		Connection connection = getConnection();
		Statement statement = connection.createStatement();
		statement.execute("UPDATE PEOPLE SET MFA_ENABLED = "+ (enabled ? 1 : 0) +" WHERE USER_ID = '"+ username +"'"); /* BAD - user input should always be sanitized */
	}
