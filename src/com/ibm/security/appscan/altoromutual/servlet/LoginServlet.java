...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// Password (first factor) is verified. Do NOT establish the session yet -
		// the second factor (TOTP) must be satisfied first. Clear any stale MFA
		...
		try{
			if (DBUtil.isMfaEnabled(username)){
				// Returning user: ask for the current authenticator code.
				session.setAttribute(ServletUtil.SESSION_ATTR_MFA_PENDING_USER, username);
				response.sendRedirect(request.getContextPath()+"/mfa.jsp");
			} else {
				// First login: force MFA enrollment before granting access.
				String secret = TotpUtil.generateSecret();
				session.setAttribute(ServletUtil.SESSION_ATTR_MFA_ENROLL_USER, username);
				session.setAttribute(ServletUtil.SESSION_ATTR_MFA_ENROLL_SECRET, secret);
