...
This fork adds time-based one-time password (TOTP) multi-factor authentication compatible with Google Authenticator (and any RFC 6238 app), layered on top of the existing username/password login.

**Behavior**
- **Forced enrollment on first login.** After a user's password is verified, if they have not yet enrolled they are sent to an enrollment page showing a QR code (rendered client-side) plus a manual setup key. The secret is only persisted once the user confirms it by entering a valid code.
- **Returning logins** prompt for the current 6-digit code before access is granted.
- The session is **not** authenticated until the second factor succeeds — a password-verified-but-not-yet-MFA'd session cannot reach any protected page.
- **REST API** (`POST /api/login`): send `totp` alongside `username`/`password`. Missing/invalid code returns `401 {"mfaRequired": true}`; a user who has not enrolled returns `403` (enroll via the web UI first).

**Implementation highlights**
- `util/TotpUtil.java` — secret generation, code verification (±1 time-step tolerance), and `otpauth://` URI construction (built directly, no extra HTTP-client dependency).
- `PEOPLE` table gains `TOTP_SECRET` and `MFA_ENABLED`; existing Derby databases are migrated automatically via an idempotent `ALTER TABLE` (no wipe required).
- Web flow: `LoginServlet` branch + `MfaSetupServlet` (`/doMfaSetup`) and `MfaVerifyServlet` (`/doMfaVerify`), with `mfaSetup.jsp` / `mfa.jsp` and a bundled client-side QR library.
- Dependency added: `com.warrenstrange:googleauth:1.5.0`.
...
- **JDK 17**
- **Gradle 8.9** (via the included wrapper — just run `./gradlew`)
- **Tomcat 9.x** (the app uses the `javax.*` Servlet API, so Tomcat 9, not 10+)

Build the deployable WAR with:

```
./gradlew clean build
```

The artifact is produced at `build/libs/altoromutual.war`.
